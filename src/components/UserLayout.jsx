...
const navItems = [
  ...
  { to: '/user/katalog', label: 'Katalog Produk' },
  { to: '/user/keranjang', label: 'Keranjang' },
  { to: '/user/pesanan', label: 'Pesanan Saya' },
  { to: '/user/limbah', label: 'Fitur Limbah' },
  { to: '/user/konsultasi', label: 'Konsultasi AI'},
      ...
      <aside className="hidden md:flex flex-col w-56 bg-white border-r border-gray-100 fixed top-0 left-0 h-full z-40">
        ...
        <div className="px-3 py-4 border-t border-gray-100">
          <div className="px-3 py-2 mb-2">
            <p className="text-xs font-medium text-gray-900 truncate">{profile?.nama || 'Pengguna'}</p>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40" onClick={() => setMobileOpen(false)}>
          ...
          <div className="absolute top-0 left-0 w-64 h-full bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-5 border-b border-gray-100">
              <p className="text-sm font-medium text-gray-900">{profile?.nama || 'Pengguna'}</p>
              <p className="text-xs text-gray-400">Pengguna</p>
            </div>
